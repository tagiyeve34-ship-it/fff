...
class MainActivity : AppCompatActivity() {
    ...
    private fun loadInitialChild() {
        lifecycleScope.launch {
            try {
                val response = ApiClient.service(this@MainActivity).children()
                val firstChild = response.body()?.children?.firstOrNull()
                if (firstChild != null && Prefs.child(this@MainActivity) < 0) {
                    Prefs.setChild(this@MainActivity, firstChild.id)
                }
            } catch (_: Exception) {
            }
        }
    ...
    private fun registerPushToken() {
        FirebaseMessaging.getInstance().token.addOnSuccessListener { token ->
            ...
            lifecycleScope.launch {
                try {
                    ApiClient.service(this@MainActivity)
                        .registerPush(PushRegisterRequest(token))
                } catch (_: Exception) {
                }
            }
        }
